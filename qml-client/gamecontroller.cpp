...
void GameController::onNetworkGameStarted(int seed, int numTeams, bool teamMode) {
    // NetworkClient already initialized its m_game. Copy the state.
...
void GameController::onNetworkMoveReceived(QJsonObject moveData) {
    ...
    // Sync state from network client (it already applied the move)
    m_game.state() = m_networkClient->game().state();
    ...
    QString team = moveData.contains("team") ?
        QString::fromUtf8(teamName(static_cast<Team>(moveData["team"].toInt()))) : "?";
    addLog(QString("[%1] сетевой ход").arg(team));
...
bool GameController::isAITurn() const {
    ...
    // In network game, block input if it's not our team's turn
    if (m_isNetworkGame && m_myNetworkTeam >= 0) {
        return static_cast<int>(m_game.currentTeam()) != m_myNetworkTeam;
    }
...
void GameController::tryExecuteMove(Coord target) {
    ...
    if (!m_isNetworkGame) scheduleAIIfNeeded();
}
